import React, { useEffect } from "react";
import { gsap } from "gsap";

export default function DropAnimation() {
  useEffect(() => {
    const drops = gsap.utils.toArray(".drop");

    const k = Math.random() * 10;
    drops.forEach((drop, i) => {
      gsap.fromTo(
        drop,
        { y: 0, opacity: 1 },
        {
          y: 20,
          opacity: 0,
          duration: 2,
          ease: "power2.out",
          repeat: -1,
          delay: i * 0.3 * k,
        },
      );
    });
  }, []);

  return (
    <div className="flex justify-center items-center">
      <svg viewBox="0 0 36 36" className="w-48 h-48 overflow-visible">
        <path
          d="m36 18c0 9.941-8.059 18-18 18-9.94 0-18-8.059-18-18 0-9.94 8.06-18 18-18 9.941 0 18 8.06 18 18"
          fill="#0e7490"
        />
        <g fill="white">
          <ellipse cx="11.5" cy="17" rx="2.5" ry="3.5" />
          <ellipse cx="24.5" cy="17" rx="2.5" ry="3.5" />
          <path d="m5.999 13.5c-.208 0-.419-.065-.599-.2-.442-.331-.531-.958-.2-1.4 3.262-4.35 7.616-4.4 7.8-4.4.552 0 1 .448 1 1 0 .551-.445.998-.996 1-.155.002-3.568.086-6.204 3.6-.196.262-.497.4-.801.4zm24.002 0c-.305 0-.604-.138-.801-.4-2.641-3.521-6.061-3.599-6.206-3.6-.55-.006-.994-.456-.991-1.005.003-.551.447-.995.997-.995.184 0 4.537.05 7.8 4.4.332.442.242 1.069-.2 1.4-.18.135-.39.2-.599.2zm-6.516 14.879c-.011-.044-1.145-4.379-5.485-4.379s-5.474 4.335-5.485 4.379c-.053.213.044.431.232.544.188.112.433.086.596-.06.009-.008 1.013-.863 4.657-.863 3.59 0 4.617.83 4.656.863.095.09.219.137.344.137.084 0 .169-.021.246-.064.196-.112.294-.339.239-.557z" />
        </g>

        <path
          className="drop"
          d="m13.5 27.0c0 1.75-1.4 3.15-3.15 3.15s-3.15-1.4-3.15-3.15 2.52-6.3 3.15-6.3 3.15 4.55 3.15 6.3z"
          fill="#5dadec"
        />

        <path
          className="drop"
          d="m27.5 27.0c0 1.75-1.4 3.15-3.15 3.15s-3.15-1.4-3.15-3.15 2.52-6.3 3.15-6.3 3.15 4.55 3.15 6.3z"
          fill="#5dadec"
        />
      </svg>
    </div>
  );
}
